class CombatUtilities {
    ...
    static randomInt(min, max) {
        ...
        if (maxTail > minTail) {
            return Math.floor(
                min + Math.random() * (maxFloor + minTail - min + 1),
            )
        } else {
            return Math.floor(
                minCeil -
                    maxTail +
                    Math.random() * (max - (minCeil - maxTail) + 1),
            )
        }
    }
    ...
    static processAttack(source, target, abilityEffect = null) {
        ...
        switch (damageType) {
            case '/damage_types/physical':
                ...
                targetReflectPower =
                    target.combatDetails.combatStats.physicalReflectPower
                targetPenetration =
        ...
        let sourceMaxDamage =
            sourceDamageMultiplier *
            (baseDamageRatio * sourceAutoAttackMaxDamage + baseDamageFlat)
        ...
        let damageRoll = CombatUtilities.randomInt(
            sourceMinDamage,
            sourceMaxDamage,
        )
        damageRoll *= 1 + source.combatDetails.combatStats.taskDamage
        let maxPremitigatedDamage = Math.min(
            damageRoll,
        ...
        if (targetReflectPower > 0 && targetResistance > 0) {
            ...
            let sourceDamageTakenRatio =
                100 / (100 + penetratedSourceResistance)
            ...
            mitigatedReflectDamage = Math.ceil(
                sourceDamageTakenRatio * reflectDamage,
            )
            reflectDamageDone = Math.min(
                mitigatedReflectDamage,
        ...
        if (
            ...
            lifeStealHeal = source.addHitpoints(
                Math.floor(
        ...
        if (
            ...
            manaLeechMana = source.addManapoints(
                Math.floor(
        ...
        switch (combatStyle) {
            ...
            case '/combat_styles/slash':
            case '/combat_styles/smash':
                experienceGained.source.attack = this.calculateAttackExperience(
                    damageDone,
                    damagePrevented,
                    combatStyle,
                )
                experienceGained.source.power = this.calculatePowerExperience(
                    damageDone,
                    damagePrevented,
                    combatStyle,
                )
                break
            case '/combat_styles/ranged':
                experienceGained.source.ranged = this.calculateRangedExperience(
                    damageDone,
                    damagePrevented,
                )
                break
            case '/combat_styles/magic':
                experienceGained.source.magic = this.calculateMagicExperience(
                    damageDone,
                    damagePrevented,
                )
                break
        ...
        experienceGained.target.defense =
            this.calculateDefenseExperience(damagePrevented)
        experienceGained.target.stamina = this.calculateStaminaExperience(
            damagePrevented,
            damageDone,
        )

        if (mitigatedReflectDamage > 0) {
            experienceGained.target.defense += this.calculateDefenseExperience(
                mitigatedReflectDamage,
            )

            let reflectDamagePrevented = reflectDamage - reflectDamageDone

            experienceGained.source.defense = this.calculateDefenseExperience(
                reflectDamagePrevented,
            )
            experienceGained.source.stamina = this.calculateStaminaExperience(
                reflectDamagePrevented,
                reflectDamageDone,
            )
        }
